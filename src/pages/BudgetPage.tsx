      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <Card>
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs font-semibold text-primary-muted tracking-wide">MONTHLY SPENDING CAP</p>
            <button onClick={() => { setEditingCap(true); setCapAmount(monthlyCapPaise > 0 ? (monthlyCapPaise / 100).toString() : ''); }} className="text-xs font-semibold text-primary-cyan">
          ...
          {monthlyCapPaise > 0 ? (
            <>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-primary-text font-bold">{formatPaise(totalSpent.toString())}</span>
                <span className="text-primary-muted">of {formatPaise(monthlyCapPaise.toString())}</span>
              </div>
              <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all ${capUsed > 90 ? 'bg-primary-red' : capUsed > 70 ? 'bg-primary-orange' : 'bg-primary-green'}`} style={{ width: `${Math.min(capUsed, 100)}%` }} />
              </div>
              <p className="text-[10px] text-primary-muted mt-1">{capUsed}% used this month</p>
            </>
          ) : (
            <p className="text-sm text-primary-muted">No monthly cap set. Tap "Set" to add one.</p>
          )}
          {editingCap && (
            <div className="mt-3 flex gap-2">
              <div className="flex items-center border border-primary-border rounded-lg px-3 py-2 flex-1">
                <span className="text-sm text-primary-muted mr-1">₹</span>
        ...
        {/* Category Budgets */}
        <Card>
          <p className="text-xs font-semibold text-primary-muted mb-3 tracking-wide">CATEGORY BUDGETS</p>
                ...
                <div key={cat.key}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <p className="text-sm font-medium text-primary-text">{cat.key}</p>
                        {limit ? (
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-primary-muted">{formatPaise(spentPaise.toString())} / {formatPaise(limit.limitPaise.toString())}</span>
                            <button onClick={() => removeLimit(cat.key)} className="text-[10px] text-primary-red">Remove</button>
                          </div>
                        ) : (
                          <button onClick={() => { setEditingCat(cat.key); setEditAmount(''); }} className="text-xs text-primary-cyan font-semibold">Set Limit</button>
                        )}
                      </div>
                      {limit && (
                        <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden mt-1">
                          <div className={`h-full rounded-full ${usage > 90 ? 'bg-primary-red' : usage > 70 ? 'bg-primary-orange' : 'bg-primary-cyan'}`} style={{ width: `${Math.min(usage, 100)}%` }} />
                        </div>
                      )}
                    </div>
                  </div>
                  {isEditing && (
                    <div className="mt-2 ml-9 flex gap-2">
                      <div className="flex items-center border border-primary-border rounded-lg px-3 py-1.5 flex-1">
                        <span className="text-xs text-primary-muted mr-1">₹</span>
        ...
        <Card className="!p-3 bg-blue-50/50 border border-blue-100">
          <div className="flex gap-2">
            ...
            <p className="text-[11px] text-primary-navy leading-relaxed">
