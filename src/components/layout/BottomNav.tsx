...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 md:static md:translate-x-0 md:border-t-0 md:border-l md:w-auto w-full max-w-[430px] sm:max-w-2xl md:max-w-none bg-white border-t border-primary-border/50 z-50 md:hidden">
      <div className="flex md:flex-col items-center justify-around md:justify-start h-14 md:h-auto px-2 md:px-0">
        {tabs.map(tab => {
          ...
          return (
            ...
            >
              {tab.icon(active)}
              <span className={cn('text-[10px] md:text-xs font-medium', active ? 'text-primary-cyan' : 'text-primary-muted')}>{tab.label}</span>
            </button>
          );
        })}
      </div>
